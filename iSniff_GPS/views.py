...
class APList(ListView):
    ...
    def get_queryset(self):
        return AP.objects.annotate(
            num_clients=Count('client')).order_by('-num_clients')
...
class stats(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        manuf = {}
        for m in Client.objects.values_list(
...
def AppleWloc(request, bssid=None):
    ...
    if bssid in apdict.keys():
        try:
            a = AP.objects.get(
                BSSID=bssid
            )  #original design - only save ap to db if it's one that has been probed for
            (a.lat, a.lon) = apdict[bssid]
            a.save(
            )  #if Apple returns a match for BSSID we save this as location
...
def SaveDB(request, name=None):
    ...
    c.save()
    return HttpResponse('Saved %s points as %s' %
                        (len(request.session['apdict'].keys()), name))  #xss
...
def updateSSID(request):
    try:
        ...
        (lat, lon) = request.POST['position'].replace('(', '').replace(
            ')', '').split(',')
        lat = float(lat)
        lon = float(lon)
